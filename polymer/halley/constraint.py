...
class Linear(Constraint):
    ...
    def _linear_linear(self, other):
        # linear vs linear
        # are we collinear (self) or parallel (Nowhere)?
...
class Circular(Constraint):
    ...
    def _circular_planar(self, other):
        # first planar, which may provide a coplanar line
    ...
    def _circular_circular(self, other):
        # decide between coplanar and off planar solutions
...
class Spherical(Constraint):
    ...
    def _spherical_planar(self, other):
        ...
        if aa < cc - EPSILON:
            b = sqrt(cc - aa)
            return Circular(mark, mark - self.point, b)
        return Unity(mark)
